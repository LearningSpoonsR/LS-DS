...
1. Time Series Data   
    + 시간에 따라서 값이 변화하는 데이터.  
    + -> 시간에 따라서 정렬된 `data.frame`  
    + -> 1개 column이 시간 정보를 담고 있는 `data.frame`
    + -> `rownames`가 시간 정보를 담고 있는 `data.frame` -> `xts`라는 새로운 자료구조     
...
3. `dygraph`  
    + R과 몇몇 소프트웨어에서 사용가능한 시계열 시각화 패키지    
    ...
    + 링크: <https://rstudio.github.io/dygraphs/> 
  
4. `lubridate`  
    + `Date`객체등 시간을 나타내는 데이터 타입을 다루는 패키지  

# Part 1. MSFT 데이터 불러오기   

## 데이터 불러오기 (`Quandl`)   

- Quandl은 글로벌 금융 데이터 베이스 제공업체   
...
+ `xts`개체로 그래프를 그리는 시계열 그래프 특화 패키지  
+ Interactive feature 덕분에 `html`문서 (html, slidy, ioslides, flexdashboard, shiny)에 유용   
+ x축이 고정되어 있으므로 `dygraph`의 문법은 `ggplot`보다 더 쉽습니다. 
...
+ `which()`는 값이 `TRUE`인 index를 반환합니다.  
+ 즉, `available`은 `theDay-years(1)`이전의 모든 기록의 index입니다.   
+ `max(available)`을 사용해 "해당일 이전 중에서 가장 나중"의 기록을 불러옵니다.    
+ 위의 해결 방식은...  
    + 관찰값이 있던 없던 해당 시점에 사용가능한 가장 마지막 관찰값을 불러올 수 있습니다.    
    + 해당 로직은 빈번히 사용되며 모든 컴퓨터 언어와 엑셀을 사용할 때도 활용 가능합니다.  
