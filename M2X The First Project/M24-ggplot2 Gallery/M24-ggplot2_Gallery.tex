...
\hypersetup{unicode=true,
            pdftitle={M24-ggplot2 Gallery},
            pdfauthor={Learning Spoons 2019-01-20},
            pdfborder={0 0 0},
            breaklinks=true}
...
\subsubsection{Diverging Dot Plot}\label{diverging-dot-plot}
...
\includegraphics{M24-ggplot2_Gallery_files/figure-latex/unnamed-chunk-33-1.pdf}
...
\includegraphics{M24-ggplot2_Gallery_files/figure-latex/unnamed-chunk-34-1.pdf}
...
Sim: \texttt{plot\_ly}는 \texttt{ggplot2}와 문법이 매우 비슷한 시각화
패키지입니다. 아래의 코드를 실행하고 데이터셋의 이름과 변수의 이름만 잘
고치면 예쁜 파이차트를 만들 수 있습니다. (수강생 IHJ님의 아이디어로
추가했습니다.)

\url{https://plot.ly/r/} 에서 다른 \texttt{plotly} 차트들도
구경해보세요!

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# inspired by IHJ (2019-01-20)  }
\CommentTok{# https://plot.ly/r/pie-charts/}
\KeywordTok{library}\NormalTok{(plotly)}

\NormalTok{USPersonalExpenditure <-}\StringTok{ }
\StringTok{  }\KeywordTok{data.frame}\NormalTok{(}\StringTok{"Categorie"}\NormalTok{=}\KeywordTok{rownames}\NormalTok{(USPersonalExpenditure), }
\NormalTok{             USPersonalExpenditure)}
\NormalTok{data <-}\StringTok{ }\NormalTok{USPersonalExpenditure[,}\KeywordTok{c}\NormalTok{(}\StringTok{'Categorie'}\NormalTok{, }\StringTok{'X1960'}\NormalTok{)]}

\NormalTok{p <-}\StringTok{ }\KeywordTok{plot_ly}\NormalTok{(data, }\DataTypeTok{labels =} \OperatorTok{~}\NormalTok{Categorie, }\DataTypeTok{values =} \OperatorTok{~}\NormalTok{X1960, }\DataTypeTok{type =} \StringTok{'pie'}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{layout}\NormalTok{(}\DataTypeTok{title =} \StringTok{'United States Personal Expenditures by Categories in 1960'}\NormalTok{,}
         \DataTypeTok{xaxis =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{showgrid =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{zeroline =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{showticklabels =} \OtherTok{FALSE}\NormalTok{),}
         \DataTypeTok{yaxis =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{showgrid =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{zeroline =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{showticklabels =} \OtherTok{FALSE}\NormalTok{))  }
\end{Highlighting}
\end{Shaded}
...
\includegraphics{M24-ggplot2_Gallery_files/figure-latex/unnamed-chunk-42-1.pdf}

\newpage

\subsubsection{Time Series Plot For a Monthly Time
Series}\label{time-series-plot-for-a-monthly-time-series}
...
\includegraphics{M24-ggplot2_Gallery_files/figure-latex/unnamed-chunk-44-1.pdf}
